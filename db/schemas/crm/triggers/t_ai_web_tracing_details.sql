...
DROP TRIGGER IF EXISTS crm.t_ai_web_tracing_details$$

CREATE TRIGGER crm.t_ai_web_tracing_details AFTER INSERT on crm.web_tracing_details
FOR EACH ROW BEGIN 
...
SELECT new.modified_id AS app_user_id,
       ...
       'WEB_TRACED_DETAILS_INSERT' AS activity_type,
       "W" AS activity_doc_type,             
       new.web_tracing_details_id AS activity_doc_pk_id,
       CONCAT(
		CONCAT(' web_tracing_details_id was created as ', IFNULL(new.web_tracing_details_id, ''), ';'),
		CONCAT(' lead_id was created as ', IFNULL(new.lead_id, ''), ';'),
		CONCAT(' task_id was created as ', IFNULL(new.task_id, ''), ';'),
		CONCAT(' tracing_source_type_id name was created as ', IFNULL(new.tracing_source_type_id, ''), ';'),
		CONCAT(' traced_details was created as ', IFNULL(new.traced_details, ''), ';'),
...
-- Create User Notif When New Traced Details are added
   ...
   SELECT notification_type_id,
	...
	WHERE notification_type_name = "WEB_TRACED_DETAILS_ADDED";
