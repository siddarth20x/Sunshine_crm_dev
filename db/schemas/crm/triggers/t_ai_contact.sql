...
DROP TRIGGER IF EXISTS crm.t_ai_contact$$

CREATE TRIGGER crm.t_ai_contact AFTER INSERT on crm.contact
FOR EACH ROW BEGIN 
...
SELECT new.modified_id AS app_user_id,
       ...
       'CONTACT_INSERT' AS activity_type,
       "C" AS activity_doc_type,             
       new.contact_id AS activity_doc_pk_id,
       CONCAT(
		CONCAT(' contact_id was created as ', IFNULL(new.contact_id, ''), ';'),
		...
		CONCAT(' customer name was created as ', IFNULL(new.customer_name, ''), ';'),
		CONCAT(' email was created as ', IFNULL(new.email, ''), ';'),
		CONCAT(' phone was created as ', IFNULL(new.phone, ''), ';'),
		CONCAT(' phone extension was created as ', IFNULL(new.phone_ext, ''), ';'),
		CONCAT(' alternate phone was created as ', IFNULL(new.alternate_phone, ''), ';'),
...
-- Create User Notif When New contact are added
   ...
   SELECT notification_type_id,
	...
	WHERE notification_type_name = "CONTACT_ADDED";
