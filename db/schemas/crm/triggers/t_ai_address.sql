...
DROP TRIGGER IF EXISTS crm.t_ai_address$$

CREATE TRIGGER crm.t_ai_address AFTER INSERT on crm.address
FOR EACH ROW BEGIN 
...
SELECT new.modified_id AS app_user_id,
       ...
       'ADDRESS_INSERT' AS activity_type,
       "A" AS activity_doc_type,             
       new.address_id AS activity_doc_pk_id,
       CONCAT(
		CONCAT(' address_id was created as ', IFNULL(new.address_id, ''), ';'),
		...
		CONCAT(' address name was created as ', IFNULL(new.address_name, ''), ';'),
		CONCAT(' address line 1 was created as ', IFNULL(new.address_line_1, ''), ';'),
		CONCAT(' address line 2 was created as ', IFNULL(new.address_line_2, ''), ';'),
		CONCAT(' address line 3 was created as ', IFNULL(new.address_line_3, ''), ';'),
		CONCAT(' city was created as ', IFNULL(new.city, ''), ';'),
		CONCAT(' state was created as ', IFNULL(new.state, ''), ';'),
		CONCAT(' country was created as ', IFNULL(new.country, ''), ';'),
		CONCAT(' zipcode was created as ', IFNULL(new.zipcode, ''), ';'),
		CONCAT(' address type was created as ', IFNULL(new.address_type, ''), ';'),
		CONCAT(' residence type was created as ', IFNULL(new.residence_type, ''), ';'),
		CONCAT(' living status was created as ', IFNULL(new.living_status, ''), ';'),
		CONCAT(' photo was created as ', IFNULL(new.photo, ''), ';'),
		CONCAT(' current location was created as ', IFNULL(new.current_location, ''), ';'),
		CONCAT(' is primary was created as ', IFNULL(new.is_primary, ''), ';'),
...
-- Create User Notif When New ADDRESS are added
   ...
   SELECT notification_type_id,
	...
	WHERE notification_type_name = "ADDRESS_ADDED";
