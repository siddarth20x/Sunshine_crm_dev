-- call user.get_ticket_issue_category_type(@err,null);
DELIMITER $$ 
DROP PROCEDURE IF EXISTS user.get_ticket_issue_category_type$$

CREATE PROCEDURE user.get_ticket_issue_category_type(
  OUT error_code INT,
...
SET
  @q = CONCAT(
    '
SELECT  ticket_issue_category_type_id,
        ...
        modified_dtm
  FROM 
        user.ticket_issue_category_type t
 WHERE  1 = 1  '
  );
...
SET
  @q = CONCAT(
    @q,
    ' AND ticket_issue_category_type_id = ',
