...
const updateLead = async (req, res) => {
  ...
  const query = `CALL crm.create_lead (
    @err,
    ?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,@olid
  )`;
  ...
  const args = [
    ...
    team_manager_id,
    assigned_by,
    assigned_dtm || new Date().toISOString().slice(0, 19).replace('T', ' '),
    assigned_to,
    target_dtm,
    account_number,
    product_type,
...
const fetchAllCrmUserLogActivity = async (req, res) => {
  const { lead_id, start_dtm, end_dtm, filter_user_id, filter_from_date, filter_to_date } = req.query;
  
  // Extract user_id from JWT token first (preferred method)
  let user_id = null;
  if (req.user) {
    user_id = req.user.user_id || req.user.id || req.user.userId;
  }
  
  // Allow override from query params if explicitly provided (for backward compatibility)
  if (req.query.user_id) {
    user_id = req.query.user_id;
  }
  
  // If no user_id found, default to App Admin (2) for now to avoid breaking the API
  if (!user_id) {
    console.log('No user_id found in JWT token, defaulting to App Admin (2)');
    user_id = 2;
  }
  ...
  console.log("=== CRM CONTROLLER DEBUG ===");
  console.log("req.user:", req.user);
  console.log("user_id extracted:", user_id);
  console.log("args:", args);
  console.log("query:", query);
  console.log("=== END CRM CONTROLLER DEBUG ===");
  
  try {
    const { rows } = await commonController.executeQuery(query, args);
    //console.log("crm.get_activity_log::", rows);
    
    // Return results in array format that frontend expects (res.data[0])
    const allResults = rows[0] || []; // Stored procedure returns results in first array
    
    console.log(`Activity Log Results: Total ${allResults.length} activities`);
    
    return res.status(200).json({
      errorCode: 0,
      message: "Successfully retrieved activity logs",
      data: [allResults], // Wrap in array so frontend can access via res.data[0]
    });
  } catch (err) {
    console.error("crm.get_activity_log-err:::", err);
    return res.status(500).json({
      errorCode: 1,
      message: "Failed to fetch login activity logs",
      data: err,
    });
  }
};
