    ...
    .on("end", async () => {
      ...
      function convertToArrOfObjFn(arr1, arr2) {
        ...
        arr1.forEach((element, index) => {
          ...
          // Special handling for disposition stage - map to disposition_status
          const isDispositionStage = normalizedKey === "disposition_stage" || 
                                      newKey === "disposition_stage" || 
                                      element.toLowerCase().includes("disposition stage") ||
                                      element.toLowerCase().trim() === "disposition stage";
          
          if (isDispositionStage) {
            obj["disposition_status"] = arr2[index];
            obj["disposition_stage"] = arr2[index];
            newKey = "disposition_stage";
          }
...
const validateCSV = async (req) => {
  ...
  try {
    ...
    const mandatoryFields = [
      ...
      // "disposition_code"  // Commented out - not mandatory anymore
    ];
    ...
    csvData.forEach((record, rowIndex) => {
      ...
      mandatoryFields.forEach((field) => {
        // Skip the 3 special fields for now (disposition_code is no longer mandatory)
        if (
      ...
      if (cs) {
        ...
        // Commented out disposition_code validation
        // if (!dc) {
        //   recordErrors.push({
      ...
      if (record.contactable_status === 'CONTACTED') {
        if (!record.disposition_status) {
          recordErrors.push({
            field: 'contactable_status',
            message: 'For CONTACTED status, disposition status is required',
            row: rowIndex + 1
          });
        }
      }
