...
@Component({
  selector: 'app-tracing-tab',
  templateUrl: './tracing-tab.component.html',
  styleUrls: ['./tracing-tab.component.css'],
  animations: [
...
export class TracingTabComponent implements OnInit {
  ...
  moduleName: string = 'TRACING_DETAILS';
  ...
  captureRouteParams() {
    ...
    this.leadId = lead_id;
    this.getAllTracingDetails(lead_id);
  }

  getAllTracingDetails(leadId: number) {
    this.showProgressBar = true;

    const leadParams = {
      traced_details_id: null,
      lead_id: leadId,
    };

    this._sunshineAPI
      .fetchTracingDetails(leadParams)
      .then((res: any) => {
        const resData = res.data[0];
        this.myDataArray = resData.reverse();
        this.dataSource = new MatTableDataSource(this.myDataArray);
        console.log('fetchTracingDetails-->', this.myDataArray);
