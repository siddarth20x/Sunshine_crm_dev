...
// import { dailyResponse } from 'src/assets/reportsreponses/dailyreportres';
@Component({
...
export class DailyReportsComponent implements OnInit {
  ...
  stageArr: Array<string> = ['CONTACTED', 'NON CONTACTED'];
  dailyRepParams: any = {
    app_user_id: null,
    lead_id: null,
    agent_id: null,
  ...
  ngOnInit(): void {
    ...
    // let output = this.uniqueResponseHandler(dailyResponse);
    // console.log('unique-output::', output);
  }
  ...
  getAllUsers() {
    ...
    this._sunshineAPI
      ...
      .then((res: any) => {
        ...
        // this.allUsersArr = resData.filter((user: any) => {
        //   return user.reporting_to_id == this.loggedInUserId;
        // });
        console.log(this.allUsersArr);
        this.showProgressBar = false;
  ...
  generateDailyReport(appUserId: number) {
    this.showProgressBar = true;
    this.dailyRepParams.app_user_id = appUserId;

    console.log('this.dailyRepParams', this.dailyRepParams);
    this._sunshineAPI
      .dailyReports(this.dailyRepParams)
      .then((res: any) => {
        let resData = res.data[0];
        console.log('non-unique-response-daily-reports::', resData);
        // this.myDataArray = [...new Set(resData)].reverse();
        this.myDataArray = this.uniqueResponseHandler(resData);
        console.log('unique-response-daily-reports::', this.myDataArray);
  ...
  exportToExcelFormat(responseData: any[], filename: string) {
    ...
    const parseContactInfoRecords = (contactInfoString: string): any[] => {
      ...
      records.forEach(record => {
        ...
        const emailMatch = trimmedRecord.match(/email-([^\s]+)/);
        const phoneMatch = trimmedRecord.match(/phone-([^\s]+)/);
        const phoneExtMatch = trimmedRecord.match(/phone_ext-([^\s]+)/);
        const altPhoneMatch = trimmedRecord.match(/alternate_phone-([^\s]+)/);
        const contactNameMatch = trimmedRecord.match(/contact_name-([^\s]+)/);
        const relationshipMatch = trimmedRecord.match(/relationship-([^\s]+)/);
        const otherContactMatch = trimmedRecord.match(/other_contact-([^\s]+)/);
        const empStatusMatch = trimmedRecord.match(/employment_status-([^\s]+)/);
        const empTypeMatch = trimmedRecord.match(/employment_type-([^\s]+)/);
        
        parsedRecords.push({
          email: emailMatch ? emailMatch[1].trim() : null,
          phone: phoneMatch ? phoneMatch[1].trim() : null,
          phone_ext: phoneExtMatch ? phoneExtMatch[1].trim() : null,
          alternate_phone: altPhoneMatch ? altPhoneMatch[1].trim() : null,
          contact_name: contactNameMatch ? contactNameMatch[1].trim() : null,
          relationship: relationshipMatch ? relationshipMatch[1].trim() : null,
          other_contact: otherContactMatch ? otherContactMatch[1].trim() : null,
          employment_status: empStatusMatch ? empStatusMatch[1].trim() : null,
          employment_type: empTypeMatch ? empTypeMatch[1].trim() : null,
          timestamp: timestamp
        });
      });
      ...
      for (let i = 0; i < maxEntries; i++) {
        ...
        for (let key in item) {
          if (item.hasOwnProperty(key) && keyAliases.hasOwnProperty(key)) {
            ...
            if (key === 'feedback') {
              newObj[newKey] = feedbackEntries[i] ? feedbackEntries[i].replace(/^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*-\s*/, '').replace(/\s*-\s*BY:.*$/, '') : null;
            } else if (key === 'field_feedback') {
              newObj[newKey] = fieldFeedbackEntries[i] ? fieldFeedbackEntries[i].replace(/^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*-\s*/, '').replace(/\s*-\s*BY:.*$/, '') : null;
            } else if (key === 'tracing_source_type_name') {
              newObj[newKey] = tracedSourceEntries[i] ? tracedSourceEntries[i].replace(/^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*-\s*/, '').replace(/\s*-\s*BY:.*$/, '') : null;
            } else if (key === 'traced_details') {
              newObj[newKey] = tracedDetailsEntries[i] ? tracedDetailsEntries[i].replace(/^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\s*-\s*/, '').replace(/\s*-\s*BY:.*$/, '') : null;
            } else {
    ...
    const calculateColumnWidths = (data: any[]): any[] => {
      ...
      columns.forEach(column => {
        ...
        // Check content length for this column
        data.forEach(row => {
  ...
  clearFilters() {
    ...
    this.generateDailyReport(this.loggedInUserId);
  }
